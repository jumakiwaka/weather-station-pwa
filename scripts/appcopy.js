...
(function () {
  ...
  app.getForecast = function (key, label, latitude, longitude) {
    ...
    let concat = '&';
    let query = latitude && longitude ? { 'lat': latitude, 'long': longitude, 'format': 'json' } : { 'location': label, 'format': 'json' };
    let oauth = {
  ...
  app.getUserLocation = function () {
    ...
    navigator.geolocation.getCurrentPosition(function (position) {
      ...
      app.getForecast({ latitude } = position.coords, { longitude } = position.coords)
    },
      function (err) {
